...
def validate_site_option(option_value: str):
    # Typer calls this callback even when the option is None. Skip validation.
    if option_value is None:
        return option_value
    valid_sites = ["indeed", "mynimo", "jobstreet"]
    if option_value.lower() not in valid_sites:
        raise typer.BadParameter(
            f"Invalid site '{option_value}'. Please choose from: {', '.join(valid_sites)}"
        )
    return option_value.lower()
...
def main(
    skill_name: str = typer.Option(
        default=..., help="Skill or job title to search for"
    ),
    location: str = typer.Option(default=..., help="Location to search for jobs"),
    site: str = typer.Option(
        default=...,
        callback=validate_site_option,
        help="Website to scrape jobs from (options: Indeed, Mynimo, Jobstreet)",
    ),
    num_pages: int = typer.Option(
        default=...,
        callback=validate_num_pages_option,
